...
* Mapping an argument to `NULL` (as with `lambda.z` and `clast` above) drops it from the call so that it keeps its default.  Do this for any argument whose name matches an NCA parameter that this parameter does not use.  An unmapped argument named `lambda.z` is filled from the calculated `lambda.z` when one exists, and otherwise from the interval column of the same name, which is the `TRUE`/`FALSE` requesting the parameter rather than a value.
