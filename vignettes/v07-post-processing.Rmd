...
Several exclusion functions are built into PKNCA.  The built-in functions will exclude all results that either apply to the current value or are dependents of the current value (parameters that depend on it).  For example, $AUC_\infty$ depends on $\lambda_z$, and excluding based on span ratio will exclude all parameters that depend on $\lambda_z$, including $AUC_\infty$.

To see the built-in functions, type `?exclude_nca` and `?exclude_nca_by_param` at the R command line and review those help pages.  The built-in rules exclude based on a low span ratio (`exclude_nca_span.ratio()`), a high percent AUC extrapolated (`exclude_nca_max.aucinf.pext()`), too few measured concentrations for AUC calculation (`exclude_nca_count_conc_measured()`), a low half-life r-squared or adjusted r-squared (`exclude_nca_min.hl.r.squared()` and `exclude_nca_min.hl.adj.r.squared()`), and an implausibly early $T_{max}$ (`exclude_nca_tmax_early()` and `exclude_nca_tmax_0()`).  They are built on the more general `exclude_nca_by_param()`, which excludes results when a single parameter is below a minimum threshold and/or above a maximum threshold, and which can also be used directly for any parameter.  To use them, provide the function to the `FUN` argument of `exclude()` as illustrated below.  When an exclusion function returns its own reason text (as the built-in functions do), that text is used as the reason for exclusion, overriding the `reason` argument to `exclude()`.
...
To find which concentration measurements were used for the half-life ($\lambda_z$) estimation, use `get_halflife_points()`.  It returns a logical vector with one value per row of the concentration data:  `TRUE` if the point was used in the half-life fit, `FALSE` if it was not used but a half-life was calculated for the interval, and `NA` if no half-life was calculated for the interval (or if the row is excluded from all calculations).  It accepts either a PKNCAresults or a PKNCAdata object (with a PKNCAdata object, the half-life is calculated first), and the intervals do not need to start at time 0.
...
`get_halflife_fit()` gives the log-linear fit itself, one row per group and interval, as the slope and intercept of `log(conc) = intercept + slope*time`.  The slope is $-\lambda_z$.

```{r halflife-fit}
get_halflife_fit(results_obj)
```

Times in a PKNCAresults object are relative to the start of the interval, but `time_first`, `time_last`, and the time scale of `intercept` are all on the same scale as the times in the concentration data.  The fitted line can therefore be drawn with the observed concentrations without shifting anything.  `intercept` and `slope` are `NA` when the half-life could not be calculated or was excluded.

## Concentrations Along the Half-Life Fit

`get_halflife_curve()` gives concentrations along that line.  As with `stats::approx()`, give either `n` for equally-spaced times or `tout` for specific times.  Concentrations are returned as concentrations, not log-concentrations.
...
With `tout`, any times may be requested.  Extrapolation after the last concentration used for the fit is done by default, and extrapolation before the first is not, so early times give `NA`:

```{r halflife-curve-tout}
get_halflife_curve(results_obj, tout = c(0, 12, 48)) %>%
  filter(Subject == 1)
```

Both defaults can be changed with the `extrapolate_earlier` and `extrapolate_later` arguments:

```{r halflife-curve-extrapolate}
get_halflife_curve(
  results_obj,
  tout = c(0, 12, 48),
