...
1. $\lambda_z > 0$ and at the same time the selection statistic must be within a tolerance factor of the best value of that statistic.
    ...
    2. Selecting on the unadjusted $r^2$ instead requires setting `PKNCA.options("adj.r.squared.factor")` to `NA` and `PKNCA.options("r.squared.factor")` to the tolerance.  Exactly one of the two must be `NA`; setting one without the other makes every half-life calculation fail, so set them together.  Because the unadjusted $r^2$ does not reward larger windows, it usually selects fewer points than the adjusted $r^2$ does.
