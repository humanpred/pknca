...
When dosing data are given, the `aucint` parameters are dose-aware:  the doses on either side of the interval bound the profile, so a concentration measured after the next dose is never interpolated back into the interval, and a dose within the interval becomes a point to integrate to.  The method (`PPANMETH`) column reports whether the calculation was dose-aware and how the region after `tlast` was handled -- with the half-life, with `AUCall` (when `aucint.inf.obs` or `aucint.inf.pred` was requested but the half-life is not estimable), or not at all when the interval ends at or before `tlast`.
