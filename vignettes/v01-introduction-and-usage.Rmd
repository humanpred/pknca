...
The simplest analysis requires concentration data at a minimum; adding dosing data enables automatic interval determination and dose-dependent parameters (without dosing data, `intervals` must be provided to `PKNCAdata()`).  Given concentration and dosing data, it then takes five function calls to provide summarized results.  (This example runs as-is; a few later examples in this document-- the grouping and manual-interval sketches-- only illustrate the workflow without running.  For another complete example, please see [the theophylline example](v02-example-theophylline.html).)
...
Note that `PKNCAdose()` also accepts a `route` argument (`"extravascular"` or `"intravascular"`); when it is not given, extravascular administration is assumed.
...
values are coded as `NA`.  If the limit of quantification itself is available,
it can be given with the `lloq` argument to `PKNCAconc()` (a value or a column
name), and it is carried through to the censored-likelihood (Tobit) half-life
method (`hl_method = "tobit"`); see the
[Tobit half-life vignette](v06-half-life-calculation-tobit.html) for details.

Units may be assigned when creating the input objects with the unit arguments
to `PKNCAconc()` and `PKNCAdose()` (`concu`, `timeu`, `amountu`, `doseu`, and
their `*_pref` counterparts for automatic conversion).  When units are given, a
units table is automatically created during `PKNCAdata()` construction so that
calculated parameters carry their units.  See the
[unit assignment and conversion vignette](v07-unit-conversion.html) for
details.
...
summaries (see later) is less helpful because everyone could have different
...
function, and the row-level (individual) results are available with
`as.data.frame()` (which can also generate CDISC PP-style output with
...
summarization; this will include all zeros that are e.g. excluded from
...
Updating existing results is most helpful if you are writing a `shiny` or other
user interface for PKNCA. If you are using PKNCA from a script, this is not the
preferred way to perform analysis.

If you need to rerun analyses with minor updates-- changing half-life point
selection, for instance-- you can use `update()` on a `PKNCAresults` object
(`o_nca` in the example above). Using `update()` will recalculate the
minimum-required number of calculations for the data that are changed. Only
changes to the concentration or dose data are recalculated selectively; any
other change (for example, to the intervals or options) triggers a full
recalculation with a warning.
...
In these data, some participants have a nonzero predose concentration. Assume
that we want to manually change those concentrations to zero and then
recalculate. You can change the concentration data, and use `update()` to
recalculate only for the changed participants.

```{r update-example-setzero}
