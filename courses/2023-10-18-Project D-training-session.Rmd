---
title: "PKNCA Training Session for Project Dontabhaktuni"
...
## First NCA calculation with PKNCA
...
## Hands-on: Single- and Multiple-dose, single analyte: Setup the underlying datasets
...
d_multidose_single_analyte <-
  ...
  bind_rows(
    d_multidose %>%
      mutate(time=time + 120, Day=6) %>%
      left_join(d_conc %>% select(Subject, Wt, Dose) %>% unique(), by = "Subject")
  ) %>%
  mutate(
    Study_Part="Multiple"
  ) %>%
...
## Hands-on extra: Single- and Multiple-dose, single analyte: Setup the concentration and dose datasets {.smaller}
...
## Hands-on extra: Single- and Multiple-dose, single analyte: Perform basic analysis {.smaller}
...
## Hands-on extra: Single- and Multiple-dose, single analyte: Use intervals for fewer subjects {.smaller}
...
## Hands-on extra: Single- and Multiple-dose, single analyte: Use custom intervals per subjects {.smaller}
...
## Hands-on extra: Single- and Multiple-dose, parent and metabolite
